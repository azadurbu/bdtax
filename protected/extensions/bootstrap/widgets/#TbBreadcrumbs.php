...
class TbBreadcrumbs extends CBreadcrumbs
{
	...
	public function run()
	{
		...
		if ($this->homeLink !== false) {
			...
			// echo '' . $this->homeLink . $this->separator . '';
			echo '<li class="btn btn-primary"' . $active . '><i class="glyphicon glyphicon-home"></i>' . $this->homeLink . $this->separator . '</li>';
		}
		...
		foreach ($this->links as $label => $url) {
			if (is_string($label) || is_array($url)) {
				...
				switch ($i) {
					...
					break;
					case 1:
					echo '<li class="btn btn-success">';
					break;
					case 2:
					echo '<li class="btn btn-warning">';
					break;
				...
				echo strtr($this->activeLinkTemplate, array(
				 ...
				 echo '<li class="class="btn " active">';
