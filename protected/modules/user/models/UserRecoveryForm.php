...
class UserRecoveryForm extends CFormModel {
	...
	public function rules()
	{
		return array(
			...
			//array('login_or_email', 'match', 'pattern' => '/^[A-Za-z0-9@.-\s,]+$/u','message' => Yii::t('user',"Incorrect symbols (A-z0-9).")),
	...
	{
		return array(
			'login_or_email'=>Yii::t('user',"Email address"),
		);
	}
		...
		if(!$this->hasErrors())  // we only want to authenticate when no input errors
		{
			if (strpos($this->login_or_email,"@")) {
				...
				if ($user)
					...
					$this->addError("login_or_email",Yii::t('user',"Email does not exist."));
			} 
			else {

				$this->addError("login_or_email",Yii::t('user',"Email format is incorrect."));
			}
