      ...
      <div class="col-lg-8">
        <?php if ((@$ctrlName[1] != 'login') && (Yii::app()->user->name == 'Guest')) {?>
          ...
          <div class=" pull-right login-dropdown" style="margin-right: 20px;">
            ...
            <h5><?php echo Yii::t('dashboard', 'Need Help? Email:') ?> <b>support@bdtax.com.bd</b></h5>
...
//print_r($us);
         ?>
          ...
          $fromSelection = $this->userFromSelection($pensonaInfo->UserId,$this->taxYear()); 
                //echo 'Kll<pre/>';print(Yii::app()->user->CPIId);
                $activeProfile = 2;
                $profileUrl = 'personalInformation';
                $currentClass = "personalInformation";
                if(isset($fromSelection->type) && $fromSelection->type==1){
                  $activeProfile = 1;
                  $profileUrl = 'singlepage/profile';
                  $currentClass = "singlepage";
                   
                }

                //echo $ctrlName;
          ?>
          <?php if(isset($fromSelection->type) || $us->org_id): ?>
                <li <?php echo ($ctrlName == $currentClass) ? ' class="active"' : ''; ?>><?php echo CHtml::link(Yii::t('dashboard', 'Personal Information'), Yii::app()->baseUrl . "/index.php/".$profileUrl, array('class' => '')); ?></li>
          <?php endif;?>
          <?php 
            //echo $this->personalInformationStatusBar();
            if ($this->personalInformationStatusBar() >= 85) : 
            //93% is to continue without ETIN
          ?>
            <?php if(isset($fromSelection->type) && $activeProfile==1):?>

                  <li <?php echo (($ctrlName == 'incomeSingle') || ($ctrlName == 'incomeSalaries') || ($ctrlName == 'incomeHouseProperties') || ($ctrlName == 'incomeTaxRebate')) ? ' class="active"' : ''; ?>><a href='<?php echo Yii::app()->baseUrl . "/index.php/incomeSingle/startup";?>'><?php echo Yii::t('dashboard', 'Income');?></a></li>
                  <li <?php echo ($ctrlName == 'assetsSingle') ? ' class="active"' : ''; ?>><?php echo CHtml::link(Yii::t('dashboard', 'Assets & Liabilities'), Yii::app()->baseUrl . "/index.php/assetsSingle", array('class' => '')); ?></li>
                  
                  <li <?php echo ($ctrlName == 'Challan') ? ' class="active"' : ''; ?>><?php echo CHtml::link(Yii::t('dashboard', 'Bank Challan'), Yii::app()->baseUrl . "/index.php/Challan", array('class' => '')); ?></li>
                  
                  <li <?php echo ($this->action->id == 'print') ? ' class="active"' : ''; ?>><?php echo CHtml::link(Yii::t('dashboard', 'Submit'), Yii::app()->baseUrl . "/index.php/finalReview/print", array('class' => '')); ?></li>
            <?php endif; ?>
            <?php if((isset($fromSelection->type) && $activeProfile==2)):?>
        ...
        <?php if (isset(Yii::app()->user->org_id) && (Yii::app()->user->role == "companyAdmin") && (Yii::app()->user->CPIId == "companyAdmin")): ?>
          ...
          <li <?php echo (@Yii::app()->controller->module->id == 'user') ? ' class="dropdown active"' : 'dropdown'; ?> ><a title="My Users" href="<?=Yii::app()->baseUrl;?>/index.php/customers/compliancedashboard"><?php echo Yii::t('dashboard', "Compliance") ?></a></li>
        <?php endif?>
  ...
  <div id="content" class="dashbord-bg" style="padding: 0px; ff">
    ...
    <!-- FLASH MESSAGE -->
      ...
      <div style="margin-top: 10px; margin-bottom: 10px;" class="alert alert-danger alert-dismissible" role="alert">
          <?php 
             echo Yii::t('dashboard',"<b>Important Notice -</b> BDTax.com.bd has been updated according to the Finance Act of 2021. Now you can prepare your 2021-2022 tax return using our system.");

          //echo Yii::t('dashboard',"<b>Important Notice -</b> BDTax.com.bd is going through upgradation according to the Finance Act of 2019. We will notify once the upgradation is complete. Then you can prepare your 2019-2020 tax return using our system.");
          ?>
      </div>
      <?php endif; ?>
    <!-- END - FLASH MESSAGE -->
    ...
    <div class="registration" style="padding: 5px; background: #ccc; border: 1px solid #d9d9d9; margin-top: 17px;">
      <div class="dashboard-box">
        ...
        <div class="progress" style="margin: 5px ;">
          <?php if(isset($fromSelection->type) && $activeProfile==1):?>
          <div class="progress-bar progress-bar-success" role="progressbar" aria-valuenow="<?=$this->completedPercentSIngle()?>" aria-valuemin="0" aria-valuemax="100" style="min-width: 2em; width: <?=$this->completedPercentSIngle()?>%;">
            <b><?php

             echo $this->completedPercentSIngle();
             ?>%</b>
           </div>
         <?php endif; ?>
         <?php if(isset($fromSelection->type) && $activeProfile==2):?>
   ...
   <div id="content" class="dashbord-bg">
     <div class="container">
      ...
      <?php
        if(Yii::app()->user->hasFlash('alert_failed')) {
      ?>
          <div class="alert alert-danger fade in"><a href="#" data-dismiss="alert" class="close">×</a><h5><?=Yii::app()->user->getFlash('alert_failed') ?><a href="<?=Yii::app()->request->baseUrl?>/index.php/personalInformation/UpgradePackage" style="float:right;margin-top: -10px;margin-right: 10px;" class="btn btn-danger float-right">Jump to multiple page form (IT-11 GA 2016)</a></div>
      <?php
        }

        http://vault.ncprodreseller.com/index.php/admin/inventory-details/
      ?>

      <?php
        if(Yii::app()->user->hasFlash('alert_success')) {
      ?>
          <div class="alert alert-danger fade in"><a href="#" data-dismiss="alert" class="close">×</a>Please enter amount to Asset->Investments page.</div>
      <?php
        }
      ?>
...
if(!isset($fromSelection->type) && $us->role_id==2){ ?>
<script>
  $(document).ready(function() {
    $('.user-selection-page').show();
    $('.overlay-bg').show();
  });
</script>
<style type="text/css">
...
.btn-large{width: 25%;font-size: 22px;}
 ...
 <?php }else{?>
   ...
   <?php
   $pi = PersonalInformation::model()->findByPk(Yii::app()->user->CPIId);
   $isDhakCity = $this->isDhakaZone($pi);
   $userplan = $this->userCurrentPlan();
  //  $model=UserSpecialSelection::model()->find('user_id=:data AND tax_year=:data2',array(':data'=>Yii::app()->user->id, ':data2'=>$this->taxYear()) );
   if($isDhakCity && !$model && $us->role_id==2 && !isset($userplan->plan_id)){
   ?>
      ...
      $(document).ready(function() {
        ...
        $('#specialFromSlip').modal('show');
      });
      </script>
    <?php }else{ ?>

      <?php if($isDhakCity && !$model && $us->role_id==2 && ($userplan->plan_id ==1||$userplan->plan_id ==2||$userplan->plan_id ==3)){?>
...
<!-- Modal -- ------>
      ...
      <div id="" class="user-selection-page">
       <h2><?php echo Yii::t('user', "Let's check which NBR tax return form you are eligible to fill in (IT-11 GA 2016 OR IT GHA 2020)"); ?></h2>
     <h3><?php echo Yii::t('user', "Please answer the following questions before we start preparing your tax return:"); ?></h3>
         ...
         <ol>
            <li>
                <ul>
                        <li class="first"><?php echo Yii::t('user', "1. Do you have total taxable income less than 4 lac BDT"); ?>? <i class="fa fa-question-circle fa-1" /*data-toggle="tooltip"*/ title="<?=Yii::t('TTips', "101.1")?>"></i></li>
                        <li class="last">
                          <div class="btn-group btn-group-lg">
                            <button data-id="1" type="button" class="btn btn-default btn1" onclick="openFromData()"><?php echo Yii::t('user','Yes')?></button>
                            <button data-id="2" type="button" class="btn btn-big btn-default btn1"><?php echo Yii::t('user','No')?></button>
                          </div>
                        </li>
                </ul> 
            <li>
                    <ul>
                         <li class="first"><?php echo Yii::t('user', "2. Do you have gross wealth less than 40 lac BDT"); ?>? <i class="fa fa-question-circle fa-1" /*data-toggle="tooltip"*/ title="<?=Yii::t('TTips', "101.2")?>"></i></li>
                         <li class="last">
                            <div class="btn-group btn-group-lg">
                            <button data-id="1" type="button" class="btn btn-default btn2" onclick="openFromData()"><?php echo Yii::t('user','Yes')?></button>
                            <button data-id="2" type="button" class="btn btn-big btn-default btn2"><?php echo Yii::t('user','No')?></button>
                            </div>
                         </li>
                    </ul>
            <li>
                    <ul>
                          <li class="first"><?php echo Yii::t('user', "3. Do you have any motor car (jeep or microbus) in your name"); ?>? <i class="fa fa-question-circle fa-1" /*data-toggle="tooltip"*/ title="<?=Yii::t('TTips', "101.3")?>"></i></li>
                          <li class="last">

                            <div class="btn-group btn-group-lg">
                            <button data-id="1" type="button" class="btn btn-default btn3" onclick="openFromData()"><?php echo Yii::t('user','Yes')?></button>
                            <button data-id="2" type="button" class="btn btn-big btn-default btn3"><?php echo Yii::t('user','No')?></button>
                             </div>
                          </li>
                    </ul>
            </li>
            <li>
                    <ul>
                        <li class="first"><?php echo Yii::t('user', "4. Do you own/invest any house property or apartment in any city corporation area"); ?>? <i class="fa fa-question-circle fa-1" /*data-toggle="tooltip"*/ title="<?=Yii::t('TTips', "101.4")?>"></i></li>
                        <li class="last">
                          <div class="btn-group btn-group-lg">
                            <button data-id="1" type="button" class="btn btn-default btn4" onclick="openFromData()"><?php echo Yii::t('user','Yes')?></button>
                            <button data-id="2" type="button" class="btn btn-big btn-default btn4"><?php echo Yii::t('user','No')?></button>
                          </div>
                          </li>
                    </ul>
            </li>
            <li style="height:30px;"><div class="clearfix align-center"></div></li>
         </ol>

         <div class="clearfix align-center">
             <button class="btn btn-success btn-large btn-selection-save"><?php echo Yii::t('user', "Next"); ?></button>
         </div>

</div>

<div aria-hidden="true" aria-labelledby="myModalLabel" role="dialog" tabindex="-1" id="specialFromSlip" class="modal fade">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header"style="background: #B90601;padding:9px'">
                        <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                        <h4 class="modal-title"><p style="font-size: 20px;color:#000;"><strong style="color:#fff"><?php echo Yii::t('user', "বিডিট্যাক্স স্পেশাল সার্ভিস"); ?></strong></p></h4>
                    </div>
                    
                    <div class="modal-body">
                        <p style="color:#B90601;font-size: 17px;">সম্মানিত করদাতা, আপনি কি বিডিট্যাক্স টিমের ট্যাক্স এক্সপার্টের মাধ্যমে আয়কর রিটার্নটি প্রস্তুত করিয়ে নিতে ইচ্ছুক ?
                        (অর্থাৎ ট্যাক্স এক্সপার্ট আপনার সমস্ত তথ্য ইনপুট, রিভিউ, এবং সাবমিশন প্রক্রিয়া সম্পন্ন করবেন)</p>
                    </div>

                    <div class="modal-footer">
                        <button data-dismiss="modal" class="btn btn-default" onclick="redirecttoPay(2)" type="button"><?php echo Yii::t('user','না')?></button>
                        <button class="btn btn-danger" style="padding:8px 60px;" onclick="redirecttoPay(1)" type="button"><?php echo Yii::t('user','হ্যাঁ!')?></button>
                    </div>
  ...
  <div aria-hidden="true" aria-labelledby="myModalLabel" role="dialog" tabindex="-1" id="modalPlanDetails" class="modal fade">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header"style="background: #B90601;padding:9px'">
                        <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                        <h4 class="modal-title"><p style="font-size: 20px;color:#000;"><strong style="color:#fff"><?php echo Yii::t('user', "Payment Method Details"); ?></strong></p></h4>
                    </div>
                    ...
                    <div class="modal-footer">
                        <button data-dismiss="modal" class="btn btn-default"  type="button"><?php echo Yii::t('user','close')?></button>
...
<div aria-hidden="true" aria-labelledby="myModalLabel" role="dialog" tabindex="-1" id="AcknowledgementSlip" class="modal fade">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                        <h4 class="modal-title"><p style="font-size: 20px;color:#000;"><strong><?php echo Yii::t('user', "Form Selection Confirmation"); ?></strong></p></h4>
                    </div>
                    ...
                    <div class="modal-footer">
                        <button data-dismiss="modal" class="btn btn-default" type="button"><?php echo Yii::t('user','Close')?></button>
                        <button class="btn btn-primary modal-ok" type="Submit"><?php echo Yii::t('user','Ok')?></button>
                    </div>
        ...
        <style>
            .overlay-bg{position: absolute;top:-50%; background: #ccc; width: 100%;left: 0%;height: 200%;z-index: 11;opacity: 0.6;display: none;}
            .user-selection-page{position: absolute;top:20%; background: #fff; width: 80%;left: 10%;z-index: 12;display: none;}
            ...
            .user-selection-page button{ margin:0px }
            .btn-selection-save{margin-top:30px !important;}

            .align-center{text-align: center;margin-bottom: 250px;}
            .btn-lg, .btn-group-lg>.btn{font-size: 11px;font-weight: bold;}

            @media only screen and (max-width: 600px) {
            .user-selection-page ol {width: 100%;margin-top: 50px;margin-left: -25px;}
            .user-selection-page{position: absolute;top:0;}

            .user-selection-page h2,.user-selection-page h2{font-size:20px; }
            .user-selection-page ol>li{height: auto;margin-bottom: 30px;}
            .user-selection-page ul li.first{height:auto;padding: 3px;width:100%;float: left;display: block;}
            .user-selection-page ul li.last{height:auto;padding: 3px 0px 30px;width:100%;float: right;display: block;}
            .btn-selection-save{margin-top:30px;}

            .user-selection-page .btn-large{width:50% !important;}

          }

          .custom-alert{padding:8px;margin-bottom: 8px;}
        ...
        <script type="text/javascript">
            ...
            var oow ="<?php echo Yii::t('user', "1"); ?>";
            var ddw ="<?php echo Yii::t('user', "2"); ?>";
            var ttw ="<?php echo Yii::t('user', "3"); ?>";
            var ffw ="<?php echo Yii::t('user', "4"); ?>";
            var andw ="<?php echo Yii::t('user', "and"); ?>";
            
            var baseUrl = '<?php echo Yii::app()->request->baseUrl; ?>';
            var success_message ="<?php echo Yii::t('user', "Congratulations! according to  your answers, you are eligible for the one page Tax Return form, IT GHA 2020. Are you sure you would like to continue?"); ?>";
            var fail_message="<?php echo Yii::t('user', "According to your answer(s) no. <span class='answerbox'></span> you are NOT eligible for the one page form, IT GHA 2020, we are redirecting you to the regular from, IT-11 GA 2016 now. Are you sure?"); ?>";
        </script>
        <script src="<?php echo Yii::app()->request->baseUrl; ?>/js/user_selection/user_selection.js?v=1"></script>
