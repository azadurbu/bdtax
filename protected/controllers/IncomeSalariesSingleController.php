...
class IncomeSalariesSingleController extends Controller {
/**
 ...
 */
	public function actionEntry() { 
		...
		if (isset($_POST['IncomeSalaries']['NetTaxableIncome'])) {
    	$totlaIncome = $this->singleFromTaxableIncome();
    	$totlaIncome = $totlaIncome+$_POST['IncomeSalaries']['NetTaxableIncome'];
		...
		if (isset($_POST['IncomeSalaries'])) {//die('asdf');
			...
			if ($CPIInfo->GovernmentEmployee == 'Y') {
				...
				if ($model->save(false)) {
					...
					Yii::app()->user->setFlash('alert_success', Yii::t("assets","Successfully Set to No"));
...
// Uncomment the following line if AJAX validation is needed
		...
		if (isset($_POST['IncomeSalaries']['NetTaxableIncome'])) {
			$totlaIncome = $this->singleFromTaxableIncome();
	    	$totlaIncome = ($totlaIncome+$_POST['IncomeSalaries']['NetTaxableIncome'])-$model->NetTaxableIncome;
	    	if($totlaIncome>400000){
		    	$data['status'] = "failed";
				$data['msg'] = "Your income crossed 4 lac";
							Yii::app()->user->setFlash('alert_success', Yii::t("assets","Your income crossed 4 lac. Update failed!!"));
