...
class FinalReviewController extends Controller
{
	...
	public function accessRules()
	{
		return array(
			array('allow', // allow authenticated user to perform 'create' and 'update' actions
				'actions' => array('index', 'finalReview', 'finalPrint', 'viewPdf', 'finalPrintPDF', 'print', 'taxPrint', 'taxPdf', 'googleRedirect','func82bb','viewSinglePdfDetails','viewSinglePdf','viewOldPdf'),
				'users' => array('@'),
	...
	*/
	...
	public function actionViewPdf() {
		...
		if(Yii::app()->user->CPIId==3668){
			Yii::app()->user->setFlash('error', "Your download limit exists");
			$this->redirect(array('print'));
		...
		if (Yii::app()->user->role === "customers") {
			...
			//else {
				...
				if(!$userPlan && $org_plan == "Trial"){
					$this->redirect(array('/payment'));
				}
                $newplanId = $currentPlanId = $userPlan->plan_id;
                //$newplanId = $currentPlanId = 1;
				...
				if(round($this->InitialPackageIndentifier()) > 0 && isset($userSelection->type) && $userSelection->type==2){
					
                    
                    if($currentPlanId==3 || $currentPlanId==6|| $currentPlanId==7){
                        if($currentPlanId==3){
                        	$newplanId = 1;
                    ...
                    if($currentPlanId==2 || $currentPlanId==8|| $currentPlanId==9){
                        if($currentPlanId==2){
                        	$newplanId = 1;
                    ...
                    if(round($amount)<$serviceCharge && $org_plan == "Trial"){
						$this->redirect(array('/payment'));
				    }
		...
		$data82bb_info = Data82bb::model()->findByAttributes(array('userid'=>Yii::app()->user->id,'EntryYear'=>$this->taxYear(), 'cpiid'=>Yii::app()->user->CPIId));
 ...
 // $html2pdf = Yii::app()->ePdf->HTML2PDF('P','A4','en');
		...
		/*$html2pdf->WriteHTML('<p>Hello World tretert ter ert etert etertre</p>');
		$html2pdf->WriteHTML('<pagebreak />');
        $html2pdf->SetImportUse();

		$pagecount = $html2pdf->SetSourceFile(realpath('user-docs/Invoice240.pdf'));
		$tplId = $html2pdf->ImportPage($pagecount);
		//$mpdf->WriteHTML('<pagebreak />');
		$html2pdf->UseTemplate($tplId);
		...
		$html2pdf->UseTemplate($tplId);
		*/
		//$mpdf->WriteHTML('Hellllsgdgggdgdfgdgdfgdfgdfgdfgfdgfdgfdgfdgfdgfdg');
        ...
        $pdfContent = $this->render('finalPrintPDF', array(
			'personal_info_model'=>$personal_info_model,
			'income_model'=>$income_model,
			'income_salaries_model'=>$income_salaries_model,
			'income_house_model'=>$income_house_model,
        ...
        $OldTaxFile =OldTaxFile::model()->find('user_id=:data AND tax_year=:data2',array(':data'=>Yii::app()->user->id, ':data2'=>$this->taxYear() ) );
        if($OldTaxFile){
        		$OldTaxFile->pdf_content = $pdfContent;
        		$OldTaxFile->save();
        }else{
        	$OldTaxFile = new OldTaxFile;
        	$OldTaxFile->user_id = Yii::app()->user->id;
        	$OldTaxFile->tax_year = $this->taxYear();
        	$OldTaxFile->pdf_content = $pdfContent;
        	$OldTaxFile->save();
...
// ##########################   END - REFER TO A FRIEND FROM GMAIL ##########################
		...
		$data82bb = Data82bb::model()->findByAttributes(array('userid'=>Yii::app()->user->id,'EntryYear'=>$this->taxYear(), 'cpiid'=>Yii::app()->user->CPIId));
...
// Test url http://localhost/bdtax_latest/index.php/finalReview/GeneratePdfBDTAXg1g7B2Gn0oT2x0EUrN8g6bPKAtBNKv8r2NFiEfCjf7rRa4fTaBlrBDTAX?start=1&end=10&year=2017-2018&replace=no
	public function actionGeneratePdfBDTAXg1g7B2Gn0oT2x0EUrN8g6bPKAtBNKv8r2NFiEfCjf7rRa4fTaBlrBDTAXOOOOOO() {
...
###############################################################################
 ...
 // $html2pdf = Yii::app()->ePdf->HTML2PDF('P','A4','en');
	...
	function actionviewSinglePdf(){
		$this->singlePageContent();
	}

	function actionviewSinglePdfDetails(){

		$this->singlePageContent(1);
	}


	function actionviewOldPdf($tax_year){
		//die('ghu');
		...
		$html2pdf->SetDefaultFontSize('3');
		$html2pdf->setFooter('{PAGENO}');
		$OldTaxFile = OldTaxFile::model()->find('user_id=:data AND tax_year=:data2',array(':data'=>Yii::app()->user->id, ':data2'=>$tax_year) );
        if($OldTaxFile){
        	$pdfContent = $OldTaxFile->pdf_content;
        	$html2pdf->WriteHTML($pdfContent);
			$html2pdf->Output($pdfFileName,"I");
        }
	...
	function singlePageContent($details=0){
 ...
 // $html2pdf = Yii::app()->ePdf->HTML2PDF('P','A4','en');
		...
		*/
        ...
        $pdfContent = $this->render('finalPrintPDFSingle', array(
			'personal_info_model'=>$personal_info_model,
			'income_model'=>$income_model,
			'income_salaries_model'=>$income_salaries_model,
			'income_house_model'=>$income_house_model,
