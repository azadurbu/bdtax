...
<div class="alert alert-danger" style="margin:10px 13%">
    ...
    <!-- p style="color:##3c763d;font-size: 16px;" -->
      ...
      <?php if($userpk==1){?>
        ...
        <?php }elseif($userpk==3){?>
           <?=Yii::t("liability", "Thank you for purchasing our Basic service. Now please download your tax return file in PDF from Step 3, gather required documents and submit your return to your tax circle.");?>
        <?php }elseif($userpk==6){?>
          <?=Yii::t("liability", "Thank you for purchasing our Plus service. Now please upload the required documents from Step 2. Our tax experts will review your file and will get back to you for any further questions.") ?>
        <?php }else{ ?>
          <?=Yii::t("liability", "Thank you for purchasing our Premier service. Now please add your signature from Step 2 and upload the required documents from Step 3. Our tax experts will review your file and will get back to you for any further questions.") ?>
        <?php } ?>
        </p>
      </div>
<?php } ?>
...
?>
    ...
    <a class="btn btn-success" style="margin-bottom: 10px;">
      <div class="checkbox checkbox-success-82bb">
          <input id="82bbcheck" class="styled" type="checkbox"<?php if(isset($data82bb) && $data82bb->checked==1) echo 'checked'?>>
          <label for="82bbcheck">
    ...
    <?php if (Yii::app()->user->role == 'customers' && !isset(Yii::app()->user->org_id)): ?>
      ...
      ?>  
      <a href="<?php echo Yii::app()->baseUrl; ?>/index.php/payment" class="btn btn-success" style="margin-bottom: 10px;"><span><b><?=Yii::t("liability", "Step") ?> <?= ++$step ?>:</b> <?=Yii::t("liability", "Pay BDTax Fee")." (".$individual_plan->price.Yii::t("liability", " Taka Only)") ?></span>
    <?php endif; ?>

      <?php 
        if ( Yii::app()->user->role == 'customers' && $openInNewTab == true ) :
      ?>
      ...
      <?php if(isset($UserPlan) && ( $UserPlan->plan_id ==5||  $UserPlan->plan_id ==7)){?>
      <a href="<?php echo Yii::app()->baseUrl; ?>/index.php/userSignature" class="btn btn-success" style="margin-bottom: 10px;"><b><?=Yii::t("liability", "Step") ?> <?= ++$step ?>:</b> <?=Yii::t("liability", "Add Your Signature") ?>
      ...
      <?php if(isset($UserPlan) && ($UserPlan->plan_id ==4 || $UserPlan->plan_id ==5|| $UserPlan->plan_id ==6 || $UserPlan->plan_id ==7)){?>

          <a  href="<?php echo Yii::app()->baseUrl; ?>/index.php/userFile"  class="btn btn-success" style="margin-bottom: 10px;"><b><?=Yii::t("liability", "Step") ?> <?= ++$step ?>:</b> <?=Yii::t("liability", "Upload Required Documents") ?>
      ...
      <?php }else{?>
             <a data-toggle="modal" data-target="#requiredDocuments" href="#" target="_blank" class="btn btn-success" style="margin-bottom: 10px;"><b><?=Yii::t("liability", "Step") ?> <?= ++$step ?>:</b> <?=Yii::t("liability", "Gather Required Documents") ?></a>
      <?php } ?>
      
      <a href="<?php echo Yii::app()->baseUrl; ?>/index.php/finalReview/viewPdf" target="<?=( (Yii::app()->user->role == 'customers' && $openInNewTab == true) || ( (Yii::app()->user->role == 'companyAdmin' || Yii::app()->user->role == 'companyUser') && Yii::app()->user->org_plan != 'Trial') ) ? '_blank' : '_self'?>" class="btn btn-success" style="margin-bottom: 10px;"><span><b><?=Yii::t("liability", "Step") ?> <?= ++$step ?>:</b> <?=Yii::t("liability", "Download Tax Return Form (PDF)") ?></span>
      <?php 
      ...
      <?php if(isset($UserPlan) && ($UserPlan->plan_id ==6 || $UserPlan->plan_id ==7 || $UserPlan->plan_id ==4 || $UserPlan->plan_id ==5)){?>
           <a title="<?=Yii::t("liability", "Redirect to NBR website for epay") ?>" data-target="#referFriendModal" data-toggle="modal"  href="https://nbrepayment.gov.bd/micro/payIncomeTax" target="_blank" class="btn btn-success"><b><?=Yii::t("liability", "Step") ?> <?= ++$step ?>:</b> <?=Yii::t("liability", "Pay NBR through NBR payment gateway") ?></a>
      <?php }else{?>
      
          <a data-toggle="modal" data-target="#nbrAddress" href="#" target="_blank" class="btn btn-success"><b><?=Yii::t("liability", "Step") ?> <?= ++$step ?>:</b> <?=Yii::t("liability", "Find Your Local NBR Office") ?></a>
      <?php } ?> 
    </div>
...
function opennewWindow(){
  ...
  window.open("https://nbrepayment.gov.bd/micro/payIncomeTax");
}
