...
class AdminController extends Controller {
    ...
    public function actionUploadMyDoc() {
        ...
        if(isset($_POST['MyDocuments']['serial_number'])){
            $allowedFileTypes = [
                                "image/jpeg", 
                                "image/png"
                            ];
        ...
        if ( isset($_FILES['doc']['type']) && in_array($_FILES['doc']['type'], $allowedFileTypes) ) {
            ...
            if ( move_uploaded_file($_FILES['doc']['tmp_name'], $dir."/".$fileName) ) {
                ...
                $model->notes = isset($_POST['MyDocuments']['notes'])?$_POST['MyDocuments']['notes']:'';

                if(isset($_POST['MyDocuments']['serial_number'])){
                   $model->serial_number = $_POST['MyDocuments']['serial_number']; 
                   $model->is_ack = 1; 
                }
                if(isset($_POST['MyDocuments']['date_uploaded'])){
                   $model->date_uploaded = $_POST['MyDocuments']['date_uploaded']; 
                }
                ...
                if(isset($_POST['MyDocuments']['serial_number'])){
                   $url = Yii::app()->createUrl('userAckUp'); 
               }else{
    ...
    public function actionDownloadMyDoc($id,$type=0) {
        if($type==2){
            ...
            $userdocs = Yii::app()->db->createCommand("SELECT * FROM user_acknowledge  Where id = ". $id)->queryRow();
            if(!isset($userdocs['user_id'])){
                throw new CHttpException(404,'The requested page does not exist.');
            }
            $user_id = $userdocs['user_id'];
    ...
    public function actionDeleteMyDoc($id,$type=0) {
        ...
        if($type==2){
            ...
            if($userdocs['order_status']!=8){

                throw new CHttpException(404,'You are not able to delete until your order status will be completed.');
            }
            Yii::app()->db->createCommand()->delete('user_files', 'id=:id', array(':id'=>$id));
        ...
        if($type==3){
            ...
            Yii::app()->db->createCommand()->delete('user_acknowledge', 'id=:id', array(':id'=>$id));
