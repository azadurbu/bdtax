...
function InterestOnSecuritiesCalculation (e) {
	...
	$('#IncInterestOnSecurities_Cost').val( (IncInterestOnSecurities_NetAmount-IncInterestOnSecurities_CommissionOrInterest) ) ;
...
function save_IncomeAgriculture_fraction(id, description, cost, model, field_name) {
	...
	$.ajax({
		...
		data : {
			...
			IncIncomeAgriculture_CorpseType					: $("#IncIncomeAgriculture_CorpseType").val(),
...
function edit_IncomeAgriculture(id, model, field) {
	...
	$.ajax({
		...
		success : function(data) {					
			...
			$("#IncIncomeAgriculture_CorpseType").val(data.CorpseType);
