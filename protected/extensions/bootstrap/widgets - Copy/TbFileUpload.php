...
class TbFileUpload extends CJuiInputWidget
{
	...
	public function registerClientScript($id)
	{
        ...
        $bootstrap->registerAssetCss('fileupload/jquery.fileupload-ui.css');

		// Upgrade widget factory
		// @todo remove when jquery.ui 1.9+ is fully integrated into stable Yii versions
        $bootstrap->registerAssetJs('fileupload/vendor/jquery.ui.widget.js');
		//The Templates plugin is included to render the upload/download listings
        $bootstrap->registerAssetJs("fileupload/tmpl.min.js", CClientScript::POS_END);

		if ($this->previewImages || $this->imageProcessing) {
            $bootstrap->registerAssetJs("fileupload/load-image.min.js", CClientScript::POS_END);
            $bootstrap->registerAssetJs("fileupload/canvas-to-blob.min.js", CClientScript::POS_END);
			// gallery :) and one smile from me ;)
            $bootstrap->registerAssetCss("bootstrap-image-gallery.min.css");
            $bootstrap->registerAssetJs("bootstrap-image-gallery.min.js", CClientScript::POS_END);
		}
		//The Iframe Transport is required for browsers without support for XHR file uploads
        $bootstrap->registerAssetJs('fileupload/jquery.iframe-transport.js');
        $bootstrap->registerAssetJs('fileupload/jquery.fileupload.js');
		// The File Upload image processing plugin
		if ($this->imageProcessing) {
            $bootstrap->registerAssetJs('fileupload/jquery.fileupload-ip.js');
		}
		// The File Upload file processing plugin
		if ($this->previewImages) {
            $bootstrap->registerAssetJs('fileupload/jquery.fileupload-fp.js');
		}
		// locale
        $bootstrap->registerAssetJs('fileupload/jquery.fileupload-locale.js');
		//The File Upload user interface plugin
        $bootstrap->registerAssetJs('fileupload/jquery.fileupload-ui.js');
