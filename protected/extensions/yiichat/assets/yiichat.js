var YiiChat = function(options){
	...
	var makeSound = function(){
		var hh = document.createElement('audio');
  		hh.setAttribute('src', 'https://bdtax.com.bd/sound/notification_sound.mp3');
  		hh.play();
	}
	...
	chat.html("<div class='chat_remove'><a href='#' onClick='removeOldMessage()'><i class='fa fa-remove fa-lg pull-right' title='Delete Chat History'></i></a><a href='#' onClick='minimize_chat()'><i class='fa fa-minus fa-lg pull-right' title='Minimize Chat History'></i></a><div class='clearfix'></div></div><div class='posts'>posts</div><div class='you'>you</div><div class='log'></div>");
	var posts = chat.find('div.posts');
	var you = chat.find('div.you');
	var log = chat.find('div.log');

	you.html("<textarea id='textSend'></textarea><div class='exceded'></div>");	
	you.append("<button class='btn btn-success btn-lg btn-block'>"+options.sendButtonText+"</button>");
	posts.html("");

	var send = you.find('button');
	var msg = you.find('textarea');

	document.getElementById('textSend').onkeydown = function(event) {
	    if (event.keyCode == 13) {
