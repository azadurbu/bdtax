...
public function accessRules() {
	return array(
            array('allow', // allow authenticated user to perform 'create' and 'update' actions
            	'actions' => array('dashboard', 'admin', 'delete', 'districtParent', 'pIDetails1', 'pIDetails2', 'review', 'etin', 'exitSystem', 'alterUser','import','compliancedashboard','sendnotification','employee','downloademployee'),
            	'users' => array('@'),
...
public function loadModel($id)
{
			 ...
			 {  
			 	...
			 	if(isset($data[0]) && isset($data[1]) && isset($data[2])){
			        ...
			        while (($getData = fgetcsv($file, 10000, ",")) !== FALSE)
			         	...
			         	try {
                            ...
                            if($pUsers){
                            	...
                            	/*If Etin does not match with a existing user */
					         	$dataCount = User::model()->count('email=:data1',array(':data1'=>$getData[2]));

					         	if($dataCount){
					         		$Users = User::model()->find('email=:data', array(':data'=>@$getData[2]));
					         		
					         	}else{
					         		$Users = new User;
					         		$Users->email = $getData[2];
					         		$Users->first_name = $fname;
					         		$Users->last_name = $getData[2];
					         		$Users->status = 0;
					            }
					        }
				         	...
				         	$model_PI->Email = $getData[2];
				         	$model_PI->designation =  $getData[1];
			         ...
			         fclose($file);	
		             Yii::app()->user->setFlash('success', Yii::t("assets","Successfully Imported"));
	...
	function actiondownloademployee(){
        ...
        if($type==2||$type==4){
		    ...
		    $listuser =Yii::app()->db->createCommand("SELECT distinct(u.id) as uid,pi.Name,u.last_name, u.email,pi.designation,pi.ETIN FROM users as u inner join personal_information as pi on pi.UserId=u.id inner join my_documents as md ON md.user_id = pi.UserId Where pi.org_id = ". Yii::app()->user->org_id." and md.tax_year='".$tax_year."'" )->queryAll();
