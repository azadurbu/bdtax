	...
	public function accessRules()
	{
		return array(
			array('allow', // allow authenticated user to perform 'create' and 'update' actions
			    'actions' => array('index', 'success', 'fail', 'cancel','LoadPaymentData','LoadUpgradePaymentData','Viewplandetails'),
				'users' => array('@'),
	...
	public function actionIndex()
	{
		...
		if (Yii::app()->user->role === "customers") {
			/*if(round($this->TaxLeviableOnTotalIncome(Yii::app()->user->CPIId)) <= 0){
				//$plan = "plan='Zero Tax'";
				$plan_id = 3;
			}elseif(round($this->TaxLeviableOnTotalIncome(Yii::app()->user->CPIId)) > 0){
				//$plan = "plan='Premier'";
				$plan_id = 1;
			}*/
            ...
            if(isset($fromSelection->type) && $fromSelection->type==1){
            	...
            	if(round($this->InitialPackageIndentifier()) <= 0){
				//$plan = "plan='Zero Tax'";
					$plan_id = 2;
				}elseif(round($this->InitialPackageIndentifier()) > 0){
					//$plan = "plan='Premier'";
					$plan_id = 1;
				}
            }
            ...
            if($plan_id==1&&isset($_GET['prime']) && $_GET['prime']==1){
            	$Spplan_id = 10;
            }

            if($plan_id==2&&isset($_GET['prime']) && $_GET['prime']==1){
            	$Spplan_id = 12;
            }

            if($plan_id==3&&isset($_GET['prime']) && $_GET['prime']==1){
            	$Spplan_id = 11;
            }
			...
			$individualPlan =IndividualPlan::model()->findByAttributes(array('id'=>$plan_id));
			...
			$individualPlanlist =IndividualPlan::model()->findAllByAttributes(array('parent_id'=>$plan_id));
			...
			if ($payment == null) {
                ...
                $allPaymentThisYear = Payments::model()->findAll(
					array(
						'condition' => "role_id='".$role->id."' AND CPIId='".Yii::app()->user->CPIId."' AND user_id='".Yii::app()->user->id."' AND payment_year='".$this->taxYear()."' AND status
						= 'VALID' ", 
						'order' => 'payment_date DESC'
						)
					);
				...
				$this->render('individual',array(
					...
					'payment' => $payment,
					'allPaymentThisYear' => $allPaymentThisYear,
					...
					'voucher' => $voucher,
					'individualPlanlist'=>$individualPlanlist,
					'NewPlanId'=>$plan_id,
					'SpPlanId'=>$Spplan_id,
					));
                ...
                /* New Code */
                    //echo $plan_id.'kk';
				    $currentPlanId =  $this->userCurrentPlan()->plan_id;
				    $newplanId = $currentPlanId;
					if($plan_id==1 &&($currentPlanId==2 || $currentPlanId==8|| $currentPlanId==9||$currentPlanId==12)){
                        if($currentPlanId==2){
                        	$newplanId = 1;
                        }elseif($currentPlanId==8){
                        	$newplanId = 4;
                        }elseif($currentPlanId==9){
                        	$newplanId = 5;
                        }else{
                        	$newplanId = 10;
                        }
				     ...
				     if($plan_id==2 &&($currentPlanId==2 || $currentPlanId==8|| $currentPlanId==9||$currentPlanId==12)){
                        if($currentPlanId==2){
                        	$newplanId = 8;
                        }elseif($currentPlanId==8){
                        	$newplanId = 9;
				     ...
				     if($plan_id==3 &&($currentPlanId==3 || $currentPlanId==6|| $currentPlanId==7|| $currentPlanId==11)){
                        if($currentPlanId==3){
                        	$newplanId = 6;
                        }elseif($currentPlanId==6){
                        	$newplanId = 7;
                        }else{
                        	$newplanId = 11;
				     ...
				     if($plan_id==1 &&($currentPlanId==1 || $currentPlanId==4|| $currentPlanId==5|| $currentPlanId==10)){
                        if($currentPlanId==1){
                        	$newplanId = 4;
                        }elseif($currentPlanId==4){
                        	$newplanId = 5;
                        }else{
                        	$newplanId = 10;
				     ...
				     if($plan_id==1 &&($currentPlanId==3 || $currentPlanId==6|| $currentPlanId==7|| $currentPlanId==11)){
                        if($currentPlanId==3){
                        	$newplanId = 1;
                        }elseif($currentPlanId==6){
                        	$newplanId = 4;
				     ...
				     if($plan_id==2 &&($currentPlanId==3 || $currentPlanId==6|| $currentPlanId==7|| $currentPlanId==11)){
                        if($currentPlanId==3){
                        	$newplanId = 2;
                        }elseif($currentPlanId==6){
                        	$newplanId = 8;
                        }elseif($currentPlanId==7){
                        	$newplanId = 9;
                        }else{
                        	$newplanId = 10;
                        }
				   ...
				   /*if(($plan_id==1 || $plan_id==2) &&($currentPlanId==3 || $currentPlanId==6|| $currentPlanId==7)){
                        if($currentPlanId==3){
                        	$newplanId = 2;
                        }elseif($currentPlanId==6){
                        	$newplanId = 8;
				...
				//echo $plan_id.'-'.$newplanId;
				//exit();
				$individualPlan =IndividualPlan::model()->findByAttributes(array('id'=>$newplanId));
				$serviceCharge = $individualPlan->price;
				/* New Code for calculate prevuos service charage */
		...
		if ( isset($_SESSION['tran_id']) ) {
			...
			if ( isset($payment->status) && $payment->status == "VALID" ) {
				...
				$Orders =Orders::model()->findByAttributes(array(
		            'user_id'=> $payment->user_id,'tax_year'=> $this->taxYear()));
				
				$userPlan = UserPlan::model()->findByAttributes(array('user_payment_id'=>$Orders->id));
				if($userPlan->plan_id==10||$userPlan->plan_id==11||$userPlan->plan_id==12){

					$this->redirect(Yii::app()->request->baseUrl.'/index.php/userFile');
...
/* ################   END OF - STORE THIS TRANSACTION AS PENDING STATUS ################# */
	...
	public function _sendPaymentEmail($isOrg, $email, $name, $payment, $isSuccess,$plan_id) {
	                                ...
	                                <tr>
	                                    <td style="padding: 15px; text-align: justify; text-justify: inter-word; color:#444444;font-family:Open Sans,Arial,Helvetica,sans-serif; font-size:14px;">
		                                    ...
		                                    $mailBody .= '<tr>
	                                                        <th>Service Plan</th>
	                                                        <td>
	                                                            '. $individualPlan->plan .' ('.$individualPlan->description.')
	                                                        </td>
	                                                    </tr>';
	    ...
	    if($plan_id==10||$plan_id==11||$plan_id==12){
    ...
    <br /><br />
    <div>
    <p>Thank you for purchasing '.$individualPlan->plan .' ('.$individualPlan->description.'). Please login in </p>
    <p>https://bdtax.com.bd/index.php/user/login</p>
    <p>And add your signature and upload all your necessary documents as early as possible through BDTax Submit page.</p>
    <p>This will help us prioritise your file processing.</p>
    ...
    <table>
        <tr>
			...
			UserModule::sendBillingMail($email, 'Please upload all required documents for your '.$individualPlan->plan.' Service', $mailBody);
	    }
			...
			if ( strtoupper($_SERVER['REQUEST_METHOD']) == "POST" && isset($_POST['tran_id']) ) {
				...
				if ( $sslc->orderValidate($tmpPayment->tran_id, $tmpPayment->amount, $tmpPayment->currency, $_POST) ) {
					...
					if ($tmpPayment->role_id == 2) {
						...
						$this->_sendPaymentEmail(false, $user->email, $name, $payment, true, $plan_id);
					}
					else {
						$this->_sendPaymentEmail(true, $org->contact_email, $org->organization_name, $payment, true, $plan_id);
					}
					...
					$user_nps = Users::model()->findByPk($tmpPayment->user_id);

					$this->nps_survey($user_nps->email,$user_nps->first_name,$user_nps->last_name,$user_nps->mobile);
                    $this->writeIPNLog("plnid=".$plan_id." Successfully validated.");
                    $this->writeIPNLog("samount=".$amount." Successfully validated.");
					$this->placeOrder($plan_id,$amount,$tmpPayment->CPIId,$tmpPayment->user_id,$tmpPayment->role_id,$tmpPayment->payment_year);
				}
				else {
					$payment = new Payments;
					$payment->role_id = $tmpPayment->role_id;
	...
	public function nps_survey($email,$fname,$lname,$contact){
		$curl = curl_init();

		curl_setopt_array($curl, array(
		  CURLOPT_URL => "https://api.growthscore.io/v1/createcontact",
		  ...
		  CURLOPT_HTTPHEADER => array(
		    "auth-gskey: 112fd6ec50934d37931baad000aca314",
    	...
    	{
    		$ip = " IP=".@$_SERVER['REMOTE_ADDR'].", PORT=".@$_SERVER['REMOTE_PORT'];
			$filename = "NPSlog-" . date("Y.m.d") . ".txt";
	    	$content = date("d-m-Y H.i.s") . " # " . $ip . "\r\n#Email=".$email.",FName=".$fname.",LName=".$lname.",Contact=".$contact."\r\n#ResponseMsg=". $msg . "\r\n" . "\r\n";
	    	$handle = @fopen("protected/runtime/IPNListener/" . $filename, "a+");
	    	@fwrite($handle, $content);
	    	@fclose($handle);
	    	return true;
	...
	function actionLoadPaymentData(){
		...
		$post_data['tran_id'] = $_SESSION['tran_id'];
		$post_data['value_a'] = Yii::app()->user->email;
		$post_data['total_amount'] = $serviceCharge-$amount;

			if($post_data['total_amount']){
			//round($serviceCharge - $amount);
				$user = Users::model()->findByPk(Yii::app()->user->id);
				$post_data['cus_name'] = $user->first_name." ".$user->middle_name." ".$user->last_name;
				$post_data['cus_email'] = $user->email;
				$post_data['cus_phone'] = $user->mobile;
				...
				$sslc = new SSLCommerz();
				$options = $sslc->initiate($post_data, true);
				$Innerhtml = $this->renderPartial('load_ssl_options', array('options'=>$options,'planid'=>$plan_id),true);
				echo $Innerhtml.'######'.($serviceCharge-$amount);
		    }
		
		//print_r($post_data);
	    }
		//$sslc = new SSLCommerz();
		$tmpPayment =TmpPayments::model()->findByAttributes(array('tran_id'=>$_SESSION['tran_id']));

		

		$tmpPayment->payment_date = date("Y-m-d H:i:s");
		
		$tmpPayment->amount = ($serviceCharge-$amount);
		$tmpPayment->plan_id = $plan_id;
		$tmpPayment->save();

	}


	public function ActionLoadUpgradePaymentData(){
		$this->layout='empty';
        if(isset($_POST['plan_id'])){
        $plan_id = $_POST['plan_id'];
        ...
        //$_POST['plan_id'];
		$individualPlan =IndividualPlan::model()->findByAttributes(array('id'=>$newplan));
		...
		$post_data['tran_id'] = $_SESSION['tran_id'];
		$post_data['value_a'] = Yii::app()->user->email;
		$post_data['total_amount'] = $total_amount = $serviceCharge-$amount;
			...
			//round($serviceCharge - $amount);
				...
				$sslc = new SSLCommerz();
				$options = $sslc->initiate($post_data, true);
				$individualPlanlist =IndividualPlan::model()->findAllByAttributes(array('parent_id'=>$parentid));

				$Innerhtml = $this->renderPartial('load_ssl_options_new', array('due_amount'=>$total_amount,'plan_id'=>$plan_id,'options'=>$options,'individualPlanlist'=>$individualPlanlist),true);
				echo $Innerhtml.'######'.($serviceCharge-$amount);
		    }else{
	    	    
				echo '<h3 style="color:#000;padding-bottom: 20px;">Your current service plan is:<strong> '.$individualPlan->plan.' ('.$individualPlan->description.')</strong></h2>';
	        }
	...
	public function placeOrder($plan_id,$amount,$CPIId,$user_id,$role_id,$payment_year){

		 $this->writeIPNLog("amount=".$amount." Successfully validated.");

		/* New code for payment order */
	    
        $count = Orders::model()->countByAttributes(array(
            'user_id'=> $user_id,'CPIID'=> $CPIId,'tax_year'=> $payment_year,'role_id' =>$role_id
            ));
        if($count){
        	$Orders=Orders::model()->findByAttributes(array(
            'user_id'=> $user_id,'CPIID'=> $CPIId,'tax_year'=> $payment_year,'role_id' =>$role_id
            ));
            $Orders->amount = $Orders->amount+$amount;
        }else{
            $Orders = new Orders;
		...
		$Orders->tax_year = $payment_year;
		$Orders->role_id = $role_id;
		
		$Orders->save();
		$this->writeIPNLog("Order#=".$Orders->id." Successfully Ordered.");

		$count = UserPlan::model()->countByAttributes(array(
            'user_payment_id' =>$Orders->id
            ));
        if($count){
        	$UserPlan=UserPlan::model()->findByAttributes(array(
            'user_payment_id' =>$Orders->id
            ));
            
        }else{
            $UserPlan = new UserPlan;
            $UserPlan->user_payment_id = $Orders->id;

        }
        $this->writeIPNLog("Order Plan#=".$plan_id." Successfully validated.");
	...
	function actionViewplandetails(){

		if(isset($_POST['plan_id'])){

			$individual_plan = IndividualPlan::model()->findByPk($_POST['plan_id']);

			echo '<h2>'.$individual_plan->description.' - BDT '.$individual_plan->price.'</h2>'.$individual_plan->features;
		}
