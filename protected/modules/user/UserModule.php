...
class UserModule extends CWebModule
{
	 ...
	 * @desc use email for activation user account
	 */
	public $activationMailOnFailorLogin=true;
...
//	public $returnUrl = array("/user/admin");
	public $returnUrl = array("/index.php");
...
//            'with'=>array('profile'),
	 ...
	 * @return boolean
	 */
//	public static function superRole() {
//		if(Yii::app()->user->isGuest)
//			return false;
//		else {
//			if (!isset(self::$_admin)) {
...
//	}
	...
	public static function getSuperAdmins() {
		if (!self::$_superadmin) {
			$admins = User::model()->active()->superadminuser()->findAll();
			$return_name = array();
			foreach ($admins as $admin)
				array_push($return_name,$admin->email);
			self::$_superadmin = ($return_name)?$return_name:array('');
		}
		return self::$_superadmin;
	}	
	/**
	 * Send mail method
	 */
	public static function sendRemainderMail($email,$subject,$message,$BCC) {
		Yii::import('application.vendors.*');
		require_once('AWSEmail/sendemail_aws.php');

		$from = Yii::app()->params['adminEmail'];
    	$fromName = "BDTax Admin";
    	//$BCC = Yii::app()->params['bccEmail'];
		
		//send_mail($from, $fromName, $to, $subject, $body,  $CC, $BCC, $attachment)
		$result = send_mail($from, $fromName, $email, $subject, $message,  "", $BCC, "");
	...
	public static function sendBillingMail($email, $subject, $message) {
		Yii::import('application.vendors.*');
		require_once('AWSEmail/sendemail_aws.php');

		$from = Yii::app()->params['billingEmail'];
    	$fromName = Yii::app()->params['billingEmailName'];
    	$BCC = Yii::app()->params['billingBccEmail'];
    	...
    	/*$adminEmail = Yii::app()->params['billingEmail'];
    	$bccEmail = Yii::app()->params['billingBccEmail'];
    	$EmailName = Yii::app()->params['billingEmailName'];
		...
		// Additional headers
		$headers .= 'From: '.$EmailName. ' <' . $adminEmail . '>' . "\r\n";
		$headers .= 'Bcc: ' . $bccEmail . "\r\n";
	...
	public static function sendCEOMail($email,$subject,$message) {
    	Yii::import('application.vendors.*');
		require_once('AWSEmail/sendemail_aws.php');

		$from = Yii::app()->params['CEOEmail'];
		$fromName = "Zulfikar Ali";
		$BCC = $from;
    	...
    	/*
    	$CEOEmail = Yii::app()->params['CEOEmail'];
		...
		// Additional headers
		$headers .= 'From: ' . $CEOEmail . "\r\n";
		$headers .= 'Bcc: ' . $CEOEmail . "\r\n";
	...
	/*
	//UNDER CONSTRUCTION
	public static function sendContactMail($from, $fromName, $subject, $message) {
		Yii::import('application.vendors.*');
		require_once('AWSEmail/sendemail_aws.php');

		$to = Yii::app()->params['contactEmail'];
		
		//send_mail($from, $fromName, $to, $subject, $body,  $CC, $BCC, $attachment)
		$result = send_mail($from, $fromName, $to, $subject, $message,  "", "", "");
...
/*
	public static function sendTestingMail($email,$subject,$message) {
		Yii::import('application.vendors.*');
		require_once('AWSEmail/sendemail_aws.php');

		//send_mail($to, $subject, $body,  $CC, $BCC, $attachment);
...
//                self::$_users[$id] = User::model()->with(array('profile'))->findbyPk($id);
	...
	public static function getUserByEmail($email) {
		if (!isset(self::$_userByName[$email])) {
			$_userByName[$email] = User::model()->findByAttributes(array('email'=>$email));
		}
		return $_userByName[$email];
	}
