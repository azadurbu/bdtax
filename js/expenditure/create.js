...
function save_expenditure(field_name) {
	...
	if(field_name == "TotalTaxPaidLastYear") {
		var value = $("#Expenditure_"+field_name).val();
	}
	else {
		var value = $("#Expenditure_"+field_name+"Total").val();
	}

	var comment=$("#Expenditure_"+field_name+"Comment").val();
...
function save_expenditure_fraction(id, description, cost, model, field_name,comment) {
   ...
   //alert(comment);
	$.ajax({
		url : expenditure_url+"saveFractionInfo",
		type : "POST",
		dataType : "json",
		cache : false,
		data : {
...
function edit_exp(id, model, field) {
	$('#loading').css('display','block');
	$.ajax({
		url : expenditure_url+"getDataForEdit",
		type : "POST",
		dataType : "json",
		cache : false,
		data : {
...
bootbox.confirm("Are you sure?", function(result) {
  if(result) {
  	$('#loading').css('display','block');
	$.ajax({
		url : expenditure_url+"deleteData",
		type : "POST",
		dataType : "json",
		cache : false,
		data : {
...
bootbox.confirm("Are you sure?", function(result) {
  	if(result) {
		$('#loading').css('display','block');
		$.ajax({
			url : expenditure_url+"deleteParticularFieldData",
			type : "POST",
			dataType : "json",
			cache : false,
			data : {
