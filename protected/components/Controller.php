...
class Controller extends CController {
		...
		if ( isset(Yii::app()->user->role) && (Yii::app()->user->role == "companyAdmin" || Yii::app()->user->role == "companyUser") && isset(Yii::app()->user->org_id) ) {
			...
			if ($org->org_plan == "Trial") {
    			...
    			if ( $days > $trialPeriod ) {
    				...
    				$allowURL = ["customers/dashboard","customers/compliancedashboard","customers/employee","customers/import","customers/sendnotification","customers/editEmployee","customers/delete","customers/downloademployee","user/logout", "organizations/update/", "customers/sendremainder","Voucher/redeem", "user/admin/update/"];
	...
	function getUserFiles(){
    	...
    	$userPlan = UserFile::model()->findAllByAttributes(array('user_payment_id'=>$userPayId));
    	return $userPlan; 

	}

	function userCurrentSignature($userId=0){
		if(!$userId){
			$userId = Yii::app()->user->id;
		}
    	//$userPayId = $this->getUserPayId();
    	$userSignature = UserSignature::model()->findByAttributes(array('user_id'=>$userId));
    	return $userSignature; 
    }

    function userCurrentPlan(){
    	$userPayId = $this->getUserPayId();
    	$userPlan = UserPlan::model()->findByAttributes(array('user_payment_id'=>$userPayId));
    	return $userPlan; 
    }

	function getUserPayId(){
        $role = Role::model()->find('role=:data',array(':data'=> Yii::app()->user->role));
        
	    $count = Orders::model()->countByAttributes(array(
            'user_id'=> Yii::app()->user->id,'CPIID'=> Yii::app()->user->CPIId,'tax_year'=> $this->taxYear(),'role_id' =>$role->id
            ));
        if($count){
        	$Orders = Orders::model()->findByAttributes(array(
            'user_id'=> Yii::app()->user->id,'CPIID'=> Yii::app()->user->CPIId,'tax_year'=> $this->taxYear(),'role_id' =>$role->id
            ));
	...
	public function userFromSelection($user_id,$taxYear){
		return UserFromSelection::model()->findbyattributes(array('user_id' => $user_id, 'tax_year' => $taxYear));
	}
	...
	public function singleFromTaxableIncome($ot=0){
		...
		$totlaIncome = $this->totalIncome(Yii::app()->user->CPIId);
        if($ot){
        	return ($totlaIncome - (@$IncomeOtherSourceData->SanchaypatraIncome+@$IncomeOtherSourceData->Others_1));
        }
		//return ($totlaIncome - @$IncomeOtherSourceData->SanchaypatraIncome);
		return $totlaIncome;
	}

	public function totalIncome($id,$taxYear='') {
        
        if($taxYear==''){
        	$taxYear = $this->taxYear();
        }

		$model = Income::model()->find('CPIId=:data AND EntryYear=:data2', array(':data' => $id, ':data2' => $taxYear));
	...
	function totalOutputMain($val,$cpiid='',$taxYear='') {
		...
		$model = $this->modelByCPIId($cpiid,$taxYear);
	...
	function getallCaptialGainTds($IncomeId){

		$CapitalAll = Yii::app()->db->createCommand()
		    ->select('SUM(tds_amount) as total')
		    ->from('inc_income_capital_gains ')
		    ->where('IncomeId=:IncomeId', array(':IncomeId'=>$IncomeId))
		    ->queryRow();
		$CapitalShare = Yii::app()->db->createCommand()
		    ->select('SUM(tds_amount) as total')
		    ->from('inc_income_capital_gains ')
		    ->where('IncomeId=:IncomeId', array(':IncomeId'=>$IncomeId))
		    ->Andwhere('Type=:Type', array(':Type'=>'Sale of Share'))
		    ->queryRow();
		...
		$CapitalShare = Yii::app()->db->createCommand()
		    ...
		    //if($gain->Type=='Signing Money Received'){

		$CapitalOther = Yii::app()->db->createCommand()
		    ->select('SUM(tds_amount) as total')
		    ->from('inc_income_capital_gains ')
		    ->where('IncomeId=:IncomeId', array(':IncomeId'=>$IncomeId))
		    ->Andwhere('Type=:Type', array(':Type'=>'Others'))
		    ->queryRow();

		if($CapitalAll){

        $Total = $CapitalAll['total'] - (floatval($CapitalShare['total'])+floatval($CapitalOther['total']));
        	return $Total;
        }else{
        	return 0;
        }
	...
	function getallSingingMoneyIncome($IncomeId){
		
		$CapitalSing = Yii::app()->db->createCommand()
		    ->select('SUM(Cost) as total')
		    ->from('inc_income_capital_gains ')
		    ->where('IncomeId=:IncomeId', array(':IncomeId'=>$IncomeId))
		    ->Andwhere('Type=:Type', array(':Type'=>'Signing Money Received'))
		    ->queryRow();

		

        $Total = $CapitalSing['total'];
        return $Total;
		    //print_r($CapitalGain);
		    //exit();
		
	}
	function getallSingingMoneyTds($IncomeId){
	...
	function getallCaptialGain($IncomeId){
		$count = IncIncomeCapitalGains::model()->countByAttributes(array(
            'IncomeId'=> $IncomeId));
		if(!$count){
			return 0;
		}
	...
	public function tempPayableTaxRatio($id,$taxYear='') {
		...
		$GrandTotalPayableTax = $this->tempNetPayableTax($id,$taxYear);

		$income_model=Income::model()->find('CPIId=:data AND EntryYear=:data2',array(':data'=>$id, ':data2'=>$taxYear));
		$totalIncome = $this->totalIncome($id,$taxYear);

		if($income_model){
			$signingMoney = $this->getallSingingMoneyIncome($income_model->IncomeId);
            ...
            $totalIncome = $totalIncome - $signingMoney;
		}
		...
		// $totalAdjustment/$totalIncome;
		if($totalIncome==0 && $totalAdjustment>0){
				return $totalAdjustment;
		}
        if($totalAdjustment/$totalIncome<0.15){
        	return $totalAdjustment;
        }
	...
	public function TempPayableTax($id,$taxYear='') {
		...
		if ($CalculationModel != null && $CPIInfo != null) {
			...
			//echo 'TaxPercentAmount-Income ='.print_r($TaxPercentAmount);
			...
			$Agri = Income::model()->find('CPIId=:data AND EntryYear=:data2', array(':data' => $id, ':data2' => $taxYear));
			...
			//New change july 23 2020
			//$exculdeCapitalGain = 0;
			if($Agri){
			//$exculdeCapitalGain =  $this->getallCaptialGain($Agri->IncomeId);
		    }
			...
			$totalIncomeAbs = $totalIncome;
			if ($totalIncome > $initialTaxWaiver) {
				/* New Method Added 2020-2021 */
               // $totalIncome = $totalIncome-$exculdeCapitalGain;
			...
			//echo round(($payAbleTax + $PoultryFarmTax + $totalTdsAmount), 0);
			//die();
...
//$initialTaxWaiver = Starting minimum amount of income which is deducted from tax payable income like 220000 , 270000, 400000 etc
	...
	public function payableTax($id,$taxYear='') {
		...
		if ($CalculationModel != null && $CPIInfo != null) {
			...
			$paybale = $payAbleTax + $PoultryFarmTax + $totalTdsAmount;
			if($paybale>0 && $paybale<0.51){
				$paybale = 0.51;
			}

			return round($paybale,0);
			//return round(($payAbleTax + $PoultryFarmTax + $totalTdsAmount), 0);
	...
	public function StoredDataForCalculation($taxYear='') {
		if($taxYear==''){
			$taxYear = $this->taxYear();
		}
		return $CalculationModel = CalculationDataSource::model()->find('EntryYear=:data', array(':data' => $taxYear));
	}

	public function RebateAmount($id, $IncomeOtherSourceModel,$taxYear='') {

		//$id = Yii::app()->user->CPIId;

		if ($this->StoredDataForCalculation($taxYear) != null) {

			$model = $this->modelByCPIId($id,$taxYear); //INCOME MODEL

			if ( $model == NULL ) return 0;
			$signingMoney = $this->getallSingingMoneyIncome($model->IncomeId);

			$Y = $model->IncomeTaxRebateTotal; //Investment
			$Z = $this->totalIncome($id,$taxYear) - @$IncomeOtherSourceModel->SanchaypatraIncome; 
			$Z -= $signingMoney;
			$Z -= $model->Income82cTotal;
            
			//$LV = min( ($Z *(25/100)), 15000000, $Y );
			$LV = min( ($Z *(25/100)), 10000000, $Y );
	...
	public function ActualRebateAmount($id,$taxYear='',$temp=0) {
		$actualRebateAmount;
		if($taxYear==''){
			$taxYear= $this->taxYear();
		}
		...
		$advanceTaxOfCar = Yii::app()->db->createCommand("SELECT SUM(Cost) AS car_advance FROM income LEFT JOIN inc_income_tax_in_advance ON (inc_income_tax_in_advance.IncomeId = income.IncomeId AND Description = 'Car Advance Tax') Where income.CPIID = ". $id . " AND income.EntryYear = '" . $taxYear."'")->queryRow()['car_advance'];

		$model = Income::model()->findbyattributes(array('CPIId' => $id, 'EntryYear' => $taxYear, 'trash' => 0));
        ...
        if($temp==1){
			$X = $this->TempTaxLeviableOnTotalIncome($id,$taxYear);
		}else{
			$X = $this->TaxLeviableOnTotalIncome($id,$taxYear);
		}
		$Y = $this->RebateAmount($id, $IncomeOtherSourceModel,$taxYear);
		...
		if ($X == 0) {
            ...
            $B = $val;
            /* Start If only income in sancahypatra income added 2020 -zahid*/
			$totalIncome = $this->totalIncome($id,$taxYear);
			...
			if(isset($IncomeOtherSourceModel->SanchaypatraIncome)){
	        	  $totalIncomeExceptSabchayPtra = $totalIncome -($initialTaxWaiver+$IncomeOtherSourceModel->SanchaypatraIncome);
	        	  if($totalIncomeExceptSabchayPtra<=0){
	        	  	  $B = 0;
	        	  }
            }
            /* End If only income in sancahypatra income added 2020 -zahid*/
	...
	public function TempTaxLeviableOnTotalIncome($id,$taxYear='') {
		$CPIInfo = $this->loadCPIInfo($id,$taxYear);
		$payAbleTax = $this->TempPayableTax($id,$taxYear);
		//$taxration = $this->TemppayableTaxRatio($id,$taxYear);
		//die($payAbleTax);
		if($taxYear==''){
			$taxYear=$this->taxYear();
		}
		...
		$IncomeOtherSourceData = IncIncomeOtherSource::model()->find('IncomeId=:data', array(':data' => @$model->IncomeId));
		/* new 2020 */
		$CapitalGainTaxAmount = 0;
		
        $payAbleTax = $CapitalGainTaxAmount+$payAbleTax;
        /* new end 2020 */
		if ($IncomeOtherSourceData) {
		...
		//$signingMoneyTds = $this->getallSingingMoneyTds($model->IncomeId);
		//$TDSFromSanchaypatra = $TDSFromSanchaypatra+$signingMoneyTds;
		//END - new change 18 September 2016
	...
	public function InitialPackageIndentifier(){
		$taxYear=$this->taxYear();
		$id = Yii::app()->user->CPIId;
		$model = Income::model()->findbyattributes(array('CPIId' => $id, 'EntryYear' => $taxYear, 'trash' => 0));
		...
		return ($this->TaxLeviableOnTotalIncome($id,$taxYear)-$TDSFromSanchaypatra);
	}


	public function TaxWithoutCar($id,$taxYear='') {
		$CPIInfo = $this->loadCPIInfo($id,$taxYear);
		$payAbleTax = $this->PayableTax($id,$taxYear);
		//$taxration = $this->TemppayableTaxRatio($id,$taxYear);
		//die($payAbleTax);
		if($taxYear==''){
			$taxYear=$this->taxYear();
		...
		/* new 2020 */
		/* added 10/21/2020 for who has no income only the sancaypatra */
		$totalIncome = $this->totalIncome($id,$taxYear);

		/* end added 10/21/2020 */
		$CapitalGainTaxAmount = 0;
		...
		if ($payAbleTax == 0) {
			...
			//return (max($val,$CarTdsTotal) + $TDSFromSanchaypatra);
			return ($val+ $TDSFromSanchaypatra);
		}
		else if($payAbleTax > $val && $payAbleTax < $CarTdsTotal) {
			return ($CarTdsTotal + $TDSFromSanchaypatra);
		}
	...
	public function getCartax($id,$taxYear=''){

		if($taxYear==''){
			$taxYear=$this->taxYear();
		}

		return $advanceTaxOfCar = Yii::app()->db->createCommand("SELECT SUM(Cost) AS car_advance FROM income LEFT JOIN inc_income_tax_in_advance ON (inc_income_tax_in_advance.IncomeId = income.IncomeId AND Description = 'Car Advance Tax') Where income.CPIID = ". $id . " AND income.EntryYear = '" . $taxYear."'")->queryRow()['car_advance'];
	}

	public function TaxLeviableOnTotalIncome($id,$taxYear='') {
		$CPIInfo = $this->loadCPIInfo($id,$taxYear);
		$payAbleTax = $this->PayableTax($id,$taxYear);
		//$taxration = $this->TemppayableTaxRatio($id,$taxYear);
		//die($payAbleTax);
	...
	public function tempNetPayableTax($id,$taxYear='') {
		...
		$TaxLeviableOnTotalIncome = $this->TempTaxLeviableOnTotalIncome($id,$taxYear);
		$ActualRebateAmount = round($this->ActualRebateAmount($id,$taxYear,$temp=1));
		...
		/*********Added for tax year2019-2020***********/
  		...
  		$surCharge = $this->surCharge($GrandTotalPayableTax,$id,$taxYear);
		...
		return ($GrandTotalPayableTax+$CapitalGainTaxAmount);

		
	}

	public function netPayableTax($id,$taxYear='') {
		...
		$TaxLeviableOnTotalIncome = $this->TaxLeviableOnTotalIncome($id,$taxYear);
		$ActualRebateAmount = round($this->ActualRebateAmount($id,$taxYear));
		$morethan15 = 0;
		if($model){
				$morethan15 = $this->getCapitalGainTaxAmountMore($model->IncomeId);
		}


		if($morethan15){
	        $taxratio = $this->tempPayableTaxRatio($id,$taxYear);

	        if($taxratio>1){

	        	$data['taxamount'] = $taxratio;
	        	$data['taxabsolute'] = 1;
        ...
        $CapitalGainTaxAmount = 0;
		if($model){
			$CapitalGainTaxAmount = $this->getCapitalGainTaxAmountWithin($id,$GrandTotalPayableTax,$model->IncomeId);
			
	    }

	    $data['taxamount'] = $GrandTotalPayableTax+$CapitalGainTaxAmount;
        $data['taxabsolute'] = 0;
	...
	public function modelByCPIId($id,$taxYear = '') {
		if($taxYear ==''){
			$taxYear = $this->taxYear();
		}
...
######################################################################################
	...
	######################################################################################
	public function surCharge($tax,$cpiid='',$taxYear='') {
        if($taxYear ==''){
			$taxYear = $this->taxYear();
		}
...
################## Liabilities CALCULATION START ######################################
		...
		$lia_model = LiabilitiesController::loadModelByUserYear($cpiid, $taxYear);
...
################## Liabilities CALCULATION ENDS ######################################
		...
		$getSurCharge = SurCharge::model()->find('EntryYear=:d1 AND MinAmount < :d2 AND MaxAmount >= :d3', array('d1' => $taxYear, 'd2' => $total, 'd3' => $total));
		if($total<=500000000){
			...
			return ['sur_charge' => '', 'tax' => $t_tax, 'surchargeAmount' => $surchargeAmount];
...
################## Assets StatusBar START ######################################
	public function assetsStatusBarSingle() {
		...
		if (isset(Yii::app()->user->CPIId)) {
			...
			if(isset($model->BusinessCapitalConfirm) || (isset($model->NetWealthTotal) && $model->NetWealthTotal)){
				return 100;
			}else{
				return 0;
			}
...
//%%%%%%%%%%%%%%%%%Income StatusBar START%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	...
	function incomeStatusBarSingle($cpid = null) {

		if (isset(Yii::app()->user->CPIId)) {
			# code...

			$model = Income::model()->findbyattributes(array('CPIId' => Yii::app()->user->CPIId, 'EntryYear' => $this->taxYear(), 'trash' => 0));
			$Pinfo = PersonalInformation::model()->findbyattributes(array('CPIId' => Yii::app()->user->CPIId));
...
//%%%%%%%%%%%%%%%%%Income PIStatusBar START%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	...
	function personalInformationStatusBarSingle($cpid = null) {
...
//END%%%%%%%%%%%%%%%%%%%%Professional%%%%%%---PARTBDTAXPRO---%%%%%%%%%%%%%%%%%%%%%%%%
	...
	public function totalExemptedValue($cpiid='',$taxYear=''){
		if($taxYear==''){
			$taxYear= $this->taxYear();
		}
		if($cpiid==''){
	...
	public function completedPercentSIngle() {

		return round((($this->personalInformationStatusBarSingle() + $this->incomeStatusBarSingle() +$this->assetsStatusBarSingle())/3 ));
	}

	public function getCapitalGainTaxAmountWithin($cpiid,$payAbleTax,$incomeId){
		
		$UserOrderStatusCount = IncIncomeCapitalGains::model()->countByAttributes(array('IncomeId'=> $incomeId));
        $cost = 0;
        if($UserOrderStatusCount){
        	$IncIncomeCapitalGains=IncIncomeCapitalGains::model()->findAllByAttributes(array('IncomeId'=>$incomeId));
        	
            foreach ($IncIncomeCapitalGains as $gain) {
            	if($gain->Type!='Sale of Share' && $gain->Type!='Others'){

            		if($gain->Type=='Signing Money Received'){
            			  $cost += $gain->tds_amount;
            		}else{
            		
	            		//if($gain->MoreThanTenPercentHolder=="Within"){
	            			$cost += $gain->Cost*0.15;
	            		//}
            	    }
	...
	public function getCapitalGainTaxAmountMore($incomeId){
		//$rationPercent;
		//echo $totalIncome = $this->totalIncome($cpiid);
		$UserOrderStatusCount = IncIncomeCapitalGains::model()->countByAttributes(array('IncomeId'=> $incomeId));
        ...
        if($UserOrderStatusCount){
            ...
            foreach ($IncIncomeCapitalGains as $gain) {
            	if($gain->Type!='Sale of Share' && $gain->Type!='Others' && $gain->Type!='Signing Money Received'){

            		if($gain->MoreThanTenPercentHolder=="Morethan"){
	...
	public function outsandtingTaxLiability(){
		...
		$GrandTotalPayableTaxArray = $this->netPayableTax(Yii::app()->user->CPIId);
		//print_r($GrandTotalPayableTaxArray);
		if($GrandTotalPayableTaxArray['taxabsolute']==1){
			return $GrandTotalPayableTaxArray['taxamount'];
		}else{
			$GrandTotalPayableTax = $GrandTotalPayableTaxArray['taxamount'];
		}
		$tds = @$this->totalOutputInNumber($income_model, 'IncomeTaxDeductedAtSource') + @$income_model->Income82cTdsTotal;
        ...
        if(isset($userSelection->type) && $userSelection->type==1){
        	$totalAdjustment = $totalAdjustment - $TDSFromSanchaypatra;
            if($totalAdjustment<0){
            	$totalAdjustment  = 0;
            }
        }*/
		/*$CapitalGainTaxAmount = $this->getCapitalGainTaxAmount(Yii::app()->user->CPIId,$totalAdjustment,$income_model->IncomeId);
        $totalAdjustment = $CapitalGainTaxAmount+$totalAdjustment;*/
        //echo $tds = $this->getallCaptialGainTds($income_model->IncomeId);
        //$totalAdjustment = $totalAdjustment-$tds;
		return @$totalAdjustment;
	}
	...
	public function getAcknowledgementStatus(){
		...
		return Yii::app()->db->createCommand("SELECT * from user_acknowledge Where user_id = ". $userId . " AND tax_year = '" . $taxYear."'")->queryRow();

	}

	public function minimumTaxValueWithoutCar($id,$taxYear=''){
		$CPIInfo = $this->loadCPIInfo($id);
        ...
        /* Added 2020 */
		$grandpayabletax = $this->TaxLeviableOnTotalIncome($id,$taxYear);
		$minPayableTax = 0;
        if ($CPIInfo->AreaOfResidence == 1){
			$minPayableTax = 5000;
		} elseif ($CPIInfo->AreaOfResidence == 2){
		...
		$advanceTaxOfCar = 0;
		if($grandpayabletax==0){
			if($val<$advanceTaxOfCar){
				$minTax = $advanceTaxOfCar;
			}else{
				$minTax = $val;
			}
	    }elseif($grandpayabletax<=$minPayableTax){
	    	    $minTax = $val;
	    	    if($advanceTaxOfCar>0){
	    	    	$minTax = $minPayableTax;
	    	    }
                
	    }else{
	    	/* gather than 5000K and have car or not doesnot matter */
               $minTax = $val;
	    }
	...
	public function minimumTaxValue($id,$taxYear=''){
		$CPIInfo = $this->loadCPIInfo($id);
	...
	function emailNotifications($user_id){

		$user = User::model()->findByPk($user_id);
		$email = $user->email;
		//die();

		$pUsers = PersonalInformation::model()->find('UserId=:data', array(':data'=>$user_id));

		$pass = $this->generateRandomPassword(96);

		$org = Organizations::model()->findByPk($pUsers->org_id);
		

		$mailBody ='<div id=":fz" class="a3s" style="overflow: hidden;">
	                                    <table align="center" border="0" cellpadding="0" cellspacing="0" width="100%"> 
	                                        <tbody> <tr> <td>
	                                         <table align="center" border="0" cellpadding="0" cellspacing="0" style="background-color:#f8f8f8" width="750"> 
	                                            <tbody>
	                                             ...
	                                             <tr> <td> <h3 style="text-align:center">Dear Concern, </h3> </td> </tr> 
	                                             <tr> <td>&nbsp;</td> </tr>
	                                             <tr> <td align="center" style="color:#444444;font-family:Open Sans,Arial,Helvetica,sans-serif;font-size:13px;line-height:20px;padding:0 20px" valign="middle">"'.$org->organization_name.'" created an account for you. Please confirm your email address by clicking on the link below.</td> </tr> 
	                                             <tr> <td>&nbsp;</td> </tr>
	                                             <tr> <td align="center" valign="middle"> 
	                                               <div style="background-color:#f3402e;display:table;border-radius:5px;color:#ffffff;font-family:Open Sans,Arial,sans-serif;font-size:13px;text-transform:uppercase;font-weight:bold;padding:7px 12px;text-align:center;text-decoration:none;width:140px;margin:auto">
	                                                   ...
	                                                   <a href="https://bdtax.com.bd'.Yii::app()->baseUrl.'/index.php/user/registration/activateindv/id/'.$pass.'" style="color:#ffffff;text-decoration:none" target="_blank">Click To Confirm</a>
	                                               </div>
	                                           </td> </tr> 
	                                           <tr> <td>&nbsp;</td> </tr>
	                                           ...
	                                           <tr> <td align="center" style="color:#444444;font-family:Open Sans,Arial,Helvetica,sans-serif;font-size:13px" valign="middle">
	                                            <a href="https://bdtax.com.bd'.Yii::app()->baseUrl.'/index.php/user/registration/activateindv/id/'.$pass.'" style="color:#009307" target="_blank">https://bdtax.com.bd'.Yii::app()->baseUrl.'/index.php/user/registration/activateindv/id/'.$pass.'</a></td> </tr> 
	                                            <tr> <td>&nbsp;</td> </tr>
	                                            
	                                            <tr> <td align="center" style="color:#444444;font-family:Open Sans,Arial,Helvetica,sans-serif;font-size:13px;padding:0 20px" valign="middle">If you need any help or have any questions, please email us at <a href="mailto:support@bdtax.com.bd" style="color:#009307" target="_blank">support@bdtax.com.bd</a></td> </tr>
		...
		$subject = "Welcome to bdtax.com.bd - Please Confirm Your Email";
	   
  
	    if(UserModule::sendMail($email, $subject, $mailBody)){
            $user->notification_status=1;
	    	$user->activkey = $pass;
	    	$user->save(false);
	    	
	    	$pUsers->notification = 'Sent';
	    	$pUsers->save(false); 
	    }
	}
    ...
    function getIncomeByTaxYear(){
    	$taxYear = $this->taxYear();
		$userId = Yii::app()->user->CPIId;
		$IncomeSalaries = IncomeSalaries::model()->findByAttributes(array(
            'CPIId'=> Yii::app()->user->CPIId,'EntryYear'=> $this->taxYear()));
        //echo '<pre/>';
        //print_r($IncomeSalaries);
		$IncomeTaxRebate = IncomeTaxRebate::model()->findByAttributes(array(
            'CPIId'=> Yii::app()->user->CPIId,'EntryYear'=> $this->taxYear()));
		

		$data['LifeInsurancePremium'] = isset($IncomeTaxRebate->LifeInsurancePremium_1)?$IncomeTaxRebate->LifeInsurancePremium_1:0;
		$data['depositPensionScheme'] = isset($IncomeTaxRebate->DepositPensionScheme_1)?$IncomeTaxRebate->DepositPensionScheme_1:0;
		$data['investInStockOrShare'] = isset($IncomeTaxRebate->InvestInStockOrShare_1)?$IncomeTaxRebate->InvestInStockOrShare_1:0;
		$data['savingsCertificates'] = isset($IncomeTaxRebate->SavingsCertificates_1)?$IncomeTaxRebate->SavingsCertificates_1:0;

		$data['LifeInsurancePremium_asset'] = 0;
        $data['depositPensionScheme_asset'] = 0;
        $data['savingsCertificates_asset'] = 0;
        $data['investInStockOrShare_asset'] = 0;

		$Assets = Assets::model()->findByAttributes(array(
            'CPIId'=> Yii::app()->user->CPIId,'EntryYear'=> $this->taxYear()));
		if($Assets){
			$AssetsInvestments = AssetsInvestment::model()->findAllByAttributes(array(
            'AssetsId'=> $Assets->AssetsId));

            foreach ($AssetsInvestments as $AssetsInvestment) {
            	

	            if(isset($AssetsInvestment->InvestmentType) && $AssetsInvestment->InvestmentType=='Shares/Debentures'){
	            	$data['investInStockOrShare_asset'] = $AssetsInvestment->Cost;
	            }

	            if(isset($AssetsInvestment->InvestmentType) && $AssetsInvestment->InvestmentType=='Insurance Premium'){
	            	$data['LifeInsurancePremium_asset'] = $AssetsInvestment->Cost;
	            }

	            if(isset($AssetsInvestment->InvestmentType) && $AssetsInvestment->InvestmentType=='Saving Certificate/Unit Certificate/Bond'){
	            	$data['savingsCertificates_asset'] = $AssetsInvestment->Cost;
	            }

	            if(isset($AssetsInvestment->InvestmentType) && $AssetsInvestment->InvestmentType=='Prize Bond/Saving Scheme'){
	            	$data['depositPensionScheme_asset'] = $AssetsInvestment->Cost;
	            }
	        }

            //echo '<pre/>'; print_r($AssetsInvestment);

	    }

	    $inc = isset($IncomeSalaries->EmployersContributionProvidentFund_1)?$IncomeSalaries->EmployersContributionProvidentFund_1:0;
	    $inc2 = isset($IncomeSalaries->EmployersContributionProvidentFund)?$IncomeSalaries->EmployersContributionProvidentFund:0;

	    $data['incomeOutsideBusinessFundTotal'] = 0;

	    if($inc>$inc2){
	    	$data['incomeOutsideBusinessFundTotal'] = $inc;
	    }else{
	    	$data['incomeOutsideBusinessFundTotal'] = $inc2;
	    }

		//$data['incomeOutsideBusinessFundTotal'] = 

		$data['assetOutsideBusinessFundTotal'] = isset($Assets->OutsideBusinessFundTotal)?$Assets->OutsideBusinessFundTotal:0;

		$data['rebateOutsideBusinessFundTotal'] = isset($IncomeTaxRebate->SCECProvidentFund_1)?$IncomeTaxRebate->SCECProvidentFund_1:0;
        //print_r($data);
		return $data;
    }


    function isDhakaZone($PInfo){

    	  $dhakaCity = 0;
	      if((isset($PInfo->AreaOfResidence)&& $PInfo->AreaOfResidence==1) || Yii::app()->user->CPIId == 41988){
	          $Circle = array('1','9','10','14','15','17','19','36','41'
	            ...
	            '303','305','306','296','312','328','322','330','337','282','160','326','292','214','230','276','146','203','109','103','236','110','64','295','138','196','49','60','54','106','323','151','86','218','215','121','171','281','317','94','180','53','214','271','89','262','55','130','209','233','115');
	          $dhakaCity = 0;
	          if(($PInfo->TaxesZone>=1&&$PInfo->TaxesZone<=15) || strtolower($PInfo->TaxesZone)=="central survey"){
	        ...
	        if($dhakaCity==0){
	            $Circle = array('10','14','19','26','27','28','31','32','34','36','37','41','50','54','56','58','59','60','62','63','64','65','70','71','76','80','72','75','77','83','24');

	            if($PInfo->TaxesZone>=1&&$PInfo->TaxesZone<=4){
	                if(in_array(abs($PInfo->TaxesCircle), $Circle)){
	                  $dhakaCity = 1;
	                }
	            }
...
//OutsideBusinessFundTotal'Cash on hand','Cash at Bank','Other Deposits','Fund'