...
//            array('dob', 'date', 'format'=>'mm-dd-yyyy'),
			...
			array('id, password, email, activkey, create_at, lastvisit_at, superuser, status, address1, address2, city, state, zip_code, margin, mobile', 'safe', 'on'=>'search'),
				...
				array('id, password, email, create_at, lastvisit_at, superuser, status, city, state, zip_code, dob, mobile,from_date, to_date', 'safe', 'on'=>'search'),
				array('email', 'unique', 'message' => Yii::t('user',"This user's email address already exists.")),
				):array()));
}
...
public function search()
{
    ...
    {
        $criteria->addCondition('create_at >= "'.$this->from_date.' 00:00:01" ');
       // $criteria->condition = "create_at >= '$this->from_date'";  // date is database date column field
    }elseif(!empty($this->to_date) && empty($this->from_date))
    {
        $criteria->addCondition('create_at <=  "'.$this->to_date.' 23:59:59" ');
        //$criteria->condition = "create_at <= '$this->to_date'";
    }elseif(!empty($this->to_date) && !empty($this->from_date))
    {
        $criteria->addCondition('create_at >=  "'.$this->from_date.' 00:00:01" ');
        $criteria->addCondition('create_at <=  "'.$this->to_date.'  23:59:59" ');

        //$criteria->condition = "create_at  >= '$this->from_date' and create_at <= '$this->to_date'";
    }
