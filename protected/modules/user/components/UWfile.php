...
class UWfile {
	...
	public function init() {
		return array(
			'name'=>__CLASS__,
			'label'=>Yii::t('user','File field'),
			'fieldType'=>array('VARCHAR'),
			'params'=>$this->params,
			'paramsLabels' => array(
				'path'=>Yii::t('user','Upload path'),
			),
	...
	public function editAttribute($model,$field,$params=array()) {
		...
		.' '.CHtml::activeLabelEx($model,'[uwfdel]'.$field->varname,array('label'=>Yii::t('user','Delete file'),'style'=>'display:inline;')):'')
		;
	}
