		...
		if ($this->grid->enableSorting && $this->sortable && $this->name !== null) {
			...
			if ($sort->resolveAttribute($this->name) !== false){
                            if($sort->getDirection($this->name) === CSort::SORT_ASC){
                                $label .= ' <span class="'.($bootstrap->fontAwesomeCss ? 'fa fa-sort-asc' : 'icon-chevron-down').'"></span>';
                            } elseif($sort->getDirection($this->name) === CSort::SORT_DESC){
                                $label .= ' <span class="'.($bootstrap->fontAwesomeCss ? 'fa fa-sort-desc' : 'icon-chevron-up').'"></span>';
                            } else {
                                $label .= ' ';
                            }
                        }
