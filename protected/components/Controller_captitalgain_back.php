...
class Controller extends CController {
	...
	public function TemppayableTaxRatio($id,$taxYear='') {
		...
		if ($CalculationModel != null && $CPIInfo != null) {
			...
			if($Agri){
				$exculdeCapitalGain =  $this->getallSingingMoneyIncome($Agri->IncomeId);
			}else{
			...
			//die();
			$amount = round(($payAbleTax + $PoultryFarmTax + $totalTdsAmount), 0);
			//$rebate = $this->ActualRebateAmount($id,$taxYear);
			//$amount = $amount -$rebate;
			if($amount<=0){
				return '0.15';
			}

			return $amount/$totalIncome;
...
//$initialTaxWaiver = Starting minimum amount of income which is deducted from tax payable income like 220000 , 270000, 400000 etc
	...
	public function ActualRebateAmount($id,$taxYear='') {
		$actualRebateAmount;
		if($taxYear==''){
			$taxYear= $this->taxYear();
		}
	...
	public function TaxLeviableOnTotalIncome($id,$taxYear='') {
		...
		if($model){
		$CapitalGainTaxAmount = $this->getCapitalGainTaxAmountWithin($id,$payAbleTax,$model->IncomeId);
		$CapitalGainTaxAmount += $this->getCapitalGainTaxAmountMore($id,$taxration,$model->IncomeId);
	    }
...
//END%%%%%%%%%%%%%%%%%%%%Professional%%%%%%---PARTBDTAXPRO---%%%%%%%%%%%%%%%%%%%%%%%%
	...
	public function getCapitalGainTaxAmountMore($cpiid,$rationPercent,$incomeId){
		//$rationPercent;
