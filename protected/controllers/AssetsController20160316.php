...
class AssetsController extends Controller
{
	...
	public function actionSaveInfo()
	{
		...
		else
		{
			...
			{
				$data['status'] = "failed";
				$data['msg'] = Yii::t("assets","No data found");
				die(json_encode($data));
			}
		}
	...
	####################### UPDATE ##########################
		...
		$data['status'] = "success";
		$data['msg'] = Yii::t("assets","Successfully Stored");
	...
	public function ActionSaveFractionInfo()
	{
		...
		else
		{
			...
			{
				$data['status'] = "failed";
				$data['msg'] = Yii::t("assets","Description and Cost are required. Cost must be a number");
				die(json_encode($data));
			}
	...
	public function ActionSaveInvestFractionInfo()
	{
		...
		else
		{
			...
			{
				$data['status'] = "failed";
				$data['msg'] = Yii::t("assets","Type, Description and Cost are required. Cost must be a number");
				die(json_encode($data));
			}
	...
	{
		$data['msg'] = "";
		if(!is_numeric($_POST['NAgroPropertyValue']) || $_POST['NAgroPropertyValue'] < 0) $data['msg'] .= "<br>Property Value must be a positive number";
		...
		$_POST['m']['NAgroPropertyDescription'] = $_POST['NAgroPropertyDescription'];
		$_POST['m']['NAgroPropertyValue'] = $_POST['NAgroPropertyValue'];
	...
	{
		$data['msg'] = "";
		if(!is_numeric($_POST['AgroPropertyValue']) || $_POST['AgroPropertyValue'] < 0) $data['msg'] .= "<br>Property Value must be a positive number";
		...
		if($model->{$field."Confirm"} == "Yes") {
			if($model->{$field."FOrT"} == "Total") {
				...
				elseif($field == "OutsideBusiness") {
					$total = $model->OutsideBusinessCashInHandTotal + $model->OutsideBusinessCashAtBankTotal + $model->OutsideBusinessOtherdepositsTotal;
					return $total;
				}
				else {
