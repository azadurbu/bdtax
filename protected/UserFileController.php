...
class UserFileController extends Controller
{
	...
	public function accessRules()
	{
		return array(
			array('allow', // allow authenticated user to perform 'create' and 'update' actions
				'actions' => array('index', 'entry', 'Save'),
				'users' => array('@'),
	...
	{  
		if(isset($_POST['userfile'])){
			...
			try {
			    ...
			    $filepath = sprintf('uploaded-doc/userfiles/%s_%s', $times.uniqid(), $newfilename);
	...
	*/
	public function actionIndex()
	{
         ...
         $userOrderid = $this->getUserPayId();
         //$payments = Payments::model()->findByAttributes(array('id'=>$userOrderid));

		 $taxyear = $this->taxyear();

		 $UserIncomeStatus = Income::model()->findByAttributes(array('CPIId'=>Yii::app()->user->CPIId,'EntryYear'=>$taxyear));

         
        //exit();

         $UserFiles = Yii::app()->db->createCommand()
	     ->select('* , uf.id as ufid')
	     ->from('user_files uf')
	     ->leftjoin('file_types ft', 'uf.file_type=ft.id')
	     ->leftjoin('user_files_status ufs', 'ufs.user_payment_id=uf.user_payment_id')
	     ->where('uf.user_payment_id=:user_payment_id', array(':user_payment_id'=>$userOrderid))
	     ->queryAll();
		//$UserFiles=UserFile::model()->findByAttributes(array('user_payment_id'=>$userPayid));
		 $this->render('index',array(
			'title'=>"Title",
			'userFiles' =>$UserFiles,
			'UserIncomeStatus'=>$UserIncomeStatus
		 ));
	}

    function actionshowFile(){
		if(isset($_POST['file_path'])){
			$domainurl = 'http://localhost/bdtax/';
