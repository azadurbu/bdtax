...
class TbImageGallery extends CWidget
{
	...
	public function init()
	{
        ...
        $bootstrap->registerAssetCss('bootstrap-image-gallery' . (!YII_DEBUG ? '.min' : '') . '.css');
        $bootstrap->registerAssetJs('fileupload/load-image.min.js');
        $bootstrap->registerAssetJs('bootstrap-image-gallery' . (!YII_DEBUG ? '.min' : '') . '.js');
		if ($this->fullScreen) {
